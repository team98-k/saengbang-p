...
function changeTradeType(e) {
	...
	if (value === '월세') {
		...
		html += '		<input type="number" class="form-control valid" id="roomPrice" placeholder="숫자를 입력해주새요">';
		html += '		<div class="manwon" style="text-align: right;">만 원</div>';
		html += '		<div class="desc">보증금</div>';
		html += '		<input type="text" class="form-control valid" id="roomDeposit" style="width: 100%;">';
		html += '		<div class="manwon" style="text-align: right;">만 원</div>';
		html += '	</div>';
		html += '	<div class="roomMaintenanceCost">';
		html += '		<div class="desc">관리비</div>';
		html += '		<input type="number" class="form-control valid" id="roomMaintenanceCost" placeholder="숫자를 입력해주세요" style="width: 100%;">';
		html += '		<div class="manwon" style="text-align: right;">만 원</div>';
		html += '	</div>';
		html += '</div>';
	}
...
async function addRoom() {
	...
	let imgDivCount = imgDiv.length;
	let filesCount = 0;

	for (let i = 0; i < imgDivCount; i++) {
		if (imgDiv[i].querySelector('input[type=file]').files[0]) {
			formData.append(`roomImgList[${i}].file`, imgDiv[i].querySelector('input[type=file]').files[0]);
			formData.append(`roomImgList[${i}].roomImgSequence`, i + 1);
			filesCount++;
		} else if (imgDiv[i].querySelector('img').src && (!imgDiv[i].querySelector('img').src.includes('/imgs/plus.png'))) {
			let uuid = self.crypto.randomUUID();
			const file = await urlToFile(imgDiv[i].querySelector('img').src, `${uuid}.jpg`, 'image/*')
			formData.append(`roomImgList[${i}].file`, file);
			formData.append(`roomImgList[${i}].roomImgSequence`, i + 1);
			filesCount++;
		} else {
			continue;
	...
	const res = await fetch(`/rooms/${new URL(location.href).searchParams.get('roomNum')}`, {
		method: 'PUT',
		headers: {},
		body: formData
	});
	...
	if (res.ok && data !== 0) {
		...
		document.querySelector('#addBtn').innerText = '수정';
		alert('매물 수정 실패');
	}
}
...
window.addEventListener('load', async function () {
	...
	roomImgs = roomInfo.roomImgList;
	imgsCount = roomImgs.length;
	if (roomInfo.roomTradeType) {
		let html = ''
		$(document).ready(function () {
			$('#roomOption').selectpicker();
			if (roomInfo.roomTradeType === '월세') {
				document.querySelector('#roomTradeType').value = '월세';
				html += '<div class="box" id="monthPrice">';
				html += '	<div class="roomPrice">';
				html += '		<div class="desc">월 세</div>';
				...
				html += '</div>';
				$('#roomOption').val('월세');
			} else if (roomInfo.roomTradeType === '전세') {
				document.querySelector('#roomTradeType').value = '전세';
				html += '<div class="box" id="monthPrice">';
				html += '	<div class="roomPrice">';
				html += '		<div class="desc">전 세</div>';
				...
				html += '</div>';
				$('#roomOption').val('전세');
			} else if (roomInfo.roomTradeType === '매매') {
				document.querySelector('#roomTradeType').value = '매매';
				html += '<div class="box" id="monthPrice">';
				html += '	<div class="roomPrice">';
				html += '		<div class="desc">매 매</div>';
			...
			$('#roomOption').selectpicker('refresh');
			document.querySelector('#price1').innerHTML = html;
		});
	}
	if (roomInfo.roomType) {
		$('#roomType').val(`${roomInfo.roomType}`);
		$('#roomType').selectpicker('refresh');
	}
	if (roomInfo.roomHeating) {
		$('#roomHeating').val(`${roomInfo.roomHeating}`);
		$('#roomHeating').selectpicker('refresh');
	}
	if (roomInfo.roomDirection) {
		$('#roomDirection').val(`${roomInfo.roomDirection}`);
		$('#roomDirection').selectpicker('refresh');
	}
	if (roomInfo.roomParking) {
		$('#roomParking').val(`${roomInfo.roomParking}`);
		$('#roomParking').selectpicker('refresh');
	}
	if (roomInfo.roomEntranceType) {
		$('#roomEntranceType').val(`${roomInfo.roomEntranceType}`);
		$('#roomEntranceType').selectpicker('refresh');
	}
	if (roomInfo.roomMovingDate) {
		let hiddenPContent = document.querySelector(".hiddenPContent");
		$('#roomMovingDate').val(`${roomInfo.roomMovingDate}`);
		if (roomInfo.roomMovingDate === '날짜 선택') {
			hiddenPContent.style.display = "flex";
		} else {
			hiddenPContent.style.display = "none";
		}
		$('#roomMovingDate').selectpicker('refresh');
	}
	if (roomInfo.roomElevator) {
		$('#roomElevator').val(`${roomInfo.roomElevator}`);
		$('#roomElevator').selectpicker('refresh');
	}


	for (let key in roomInfo) {
		if (document.querySelector(`#${key}`)) {
			document.querySelector(`#${key}`).value = roomInfo[key];
		}
	}
	if (roomInfo.roomMovingDate === '즉시 입주') {
		document.querySelector('#roomMovingDate').value = '즉시 입주';
	} else {
		document.querySelector('#roomMovingDate').value = "날짜 선택";
		document.querySelector('#roomMovingDate').removeAttribute('id');
		document.querySelector(".hiddenDContent").style.display = "flex";
		document.querySelector('#roomMovingDate').value = roomInfo.roomMovingDate;
	}
	if (roomInfo.roomMaintenanceCost !== 0 && document.querySelector('#isHaveMaintenanceCost')) {
		document.querySelector('#isHaveMaintenanceCost').value = '있음';
		document.querySelector(".hiddenMContent").style.display = 'flex';
		document.querySelector('#roomMaintenanceCost').value = roomInfo.roomMaintenanceCost;
	} else if (document.querySelector('#isHaveMaintenanceCost')) {
		document.querySelector('#isHaveMaintenanceCost').value = '없음';
	}
	if (roomInfo.roomParking === '가능') {
		document.querySelector(".hiddenPContent").style.display = "flex";
	}

	let html = '';
	let i = 0;
	for (; i < imgsCount; i++) {
		const roomImg = roomInfo.roomImgList[i];
		let uuid = self.crypto.randomUUID();
		html += `<div class="imgDiv" style="position: relative;">`;
		html += `	<button onclick="removeImg(this.parentNode)" style="position: absolute; top: 5px; left: 80%; border: 1px solid">X</button>`;
		html += `	<div class="img-lable-div">`;
		html += `		<label for="${uuid}" style="margin-bottom: 0">`;
		html += `			<img class="room-imgs" src="${roomImg.roomImgPath}" width="120px" height="80px">`;
		html += `		</label>`;
		html += `	</div>`;
		html += `	<input type="file" id="${uuid}" name="chooseFile" accept="image/*" onchange="preview(this)">`;
		html += `</div>`;
	...
	html += `	<div class="img-lable-div">`;
	html += `		<label for="${uuid}" style="margin-bottom: 0">`;
	html += `			<img class="room-imgs" src="" width="120px" height="80px" onerror="this.onerror=null; this.src='/imgs/plus.png';">`;
	html += `		</label>`;
	html += `	</div>`;
	html += `	<input type="file" id="${uuid}" name="chooseFile" accept="image/*" onchange="preview(this)">`;
	html += `</div>`;
